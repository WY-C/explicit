...
def run_tutorial_step(pid, multi, is_async, visual_level, layout, title, description, log_name, step_id, screen):
    ...
    vu.draw_centered_text(screen, f"[싱글 플레이 튜토리얼 {step_id}/2] {title}", f"{description} | 'U' 키를 눌러 시작하세요", color=(0, 255, 100))
    waiting = True
    while waiting:
...
if __name__ == "__main__":
    ...
    tut_steps = [
        ...
        # (True, True, 2, "멀티 실시간(H)", "실시간 AI 하이라이트", "Tut_M_RT_H"), 
        # (True, True, 1, "멀티 실시간(N)", "실시간 AI 자연어", "Tut_M_RT_N")
    ]
    ...
    # run_main_step(pid, "simple", False, 0, "Phase1_Baseline", "베이스라인 측정", "아무 정보 없이 협업을 수행합니다", screen)
